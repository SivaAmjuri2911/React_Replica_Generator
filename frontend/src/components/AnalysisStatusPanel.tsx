import type { AnalysisJob, ScenarioSpecDraftDto } from '../types.js';

interface AnalysisStatusPanelProps {
  readonly job: AnalysisJob;
  readonly generating: boolean;
  readonly onGenerate: (specPath: string) => void;
}
...
function DraftResult({ result, specPath, generating, onGenerate }: DraftResultProps): JSX.Element {
  return (
      ...
      <dl>
        ...
        <dd>
          <ul className="path-list">
            {result.transformableRelativePaths.map((relativePath) => (
              <li key={relativePath}>
        ...
        <dd>
          <ul className="path-list">
            {result.manuallyAuthoredRelativePaths.map((relativePath) => (
              <li key={relativePath}>
                <code>{relativePath}</code>
              </li>
            ))}
...
export function AnalysisStatusPanel({ job, generating, onGenerate }: AnalysisStatusPanelProps): JSX.Element {
  return (
    <div className="job-panel">
      ...
      {job.status === 'succeeded' && job.result && job.specPath && (
        <DraftResult result={job.result} specPath={job.specPath} generating={generating} onGenerate={onGenerate} />
      )}

      {job.status === 'failed' && job.failure && (
        <div className="result-block result-failure">
      ...
      <details className="log-panel" open={job.status === 'running' || job.status === 'failed'}>
