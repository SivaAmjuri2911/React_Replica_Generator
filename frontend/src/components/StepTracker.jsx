...
/**
 * @typedef {('pending'|'running'|'succeeded'|'failed')} StepTrackerItemStatus
 */

/**
 * @typedef {object} StepTrackerItem
 * @property {string} key
 * @property {string} label
 * @property {StepTrackerItemStatus} status
 * @property {string} [currentDetail] Precomputed "currently: <file>" text — only meaningful while status is 'running'.
 */

/**
 * @typedef {object} StepTrackerProps
 * @property {string} title
 * @property {readonly StepTrackerItem[]} items
 */

/** @param {{status: StepTrackerItemStatus}} props */
function StepIcon({ status }) {
    if (status === 'running') {
        return <Spinner />;
    }
    if (status === 'succeeded') {
        return (<span className="step-icon step-icon-succeeded">
    ...
    if (status === 'failed') {
        return (<span className="step-icon step-icon-failed">
        <svg width="10" height="10" viewBox="0 0 16 16" fill="none" aria-hidden="true">
 ...
 */
/** @param {StepTrackerProps} props */
export function StepTracker({ title, items }) {
    return (<div>
      <h3 className="step-tracker-title">{title}</h3>
      <ol className="step-tracker">
        {items.map((item) => (<li key={item.key} className={`step-item step-item-${item.status}`}>
            <StepIcon status={item.status}/>
            <div>
              <div className="step-label">{item.label}</div>
              {item.status === 'running' && item.currentDetail && (<div className="step-detail" title={item.currentDetail}>
