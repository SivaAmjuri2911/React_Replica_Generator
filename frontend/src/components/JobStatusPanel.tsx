...
function OutputRow({ label, path }: { readonly label: string; readonly path: string }): JSX.Element {
  return (
    <>
      <dt>{label}</dt>
      <dd>
...
export function JobStatusPanel({ job }: JobStatusPanelProps): JSX.Element {
  return (
      ...
      {job.status === 'succeeded' && job.result && (
        <div className="result-block result-success">
          <h3>
            ...
            {job.attemptsUsed !== undefined && job.attemptsUsed > 1 && (
              <span className="muted"> (took {job.attemptsUsed} attempts — auto-corrected)</span>
            )}
          </h3>
          <dl>
