...
import type { BaseSourceFile } from '../services/scenarioSpecGeneration/ScenarioSpecGenerationService.js';
import { err, ok, type Result } from '../shared/Result.js';
import { ConfigurationError, type GenerationError } from '../domain/errors/GenerationError.js';
 ...
 */
export async function readBaseSolutionCodeFiles(
  fileSystem: FileSystemService,
  baseSolutionCode: string
): Promise<Result<BaseSourceFile[], GenerationError>> {
  const listResult = await fileSystem.listFilesRecursive(baseSolutionCode);
  if (!listResult.ok) {
    return err(listResult.error);
  }

  const files: BaseSourceFile[] = [];
