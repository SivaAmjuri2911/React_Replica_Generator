 ...
 */
export class RouteRenameConsistencyRule {
    name = 'RouteRenameConsistencyRule';
    async evaluate(context) {
        ...
        if (offenders.length > 0) {
            return err(new RuleViolationError(this.name, `A renamed page's route path was never updated to match — the base scenario's original URL literal still appears in generated code even though the component/display text was renamed. Every <Route path>, <Link to>, href, or navigate() using that path needs its own textReplacements entry (this is a separate literal from the capitalized entity name). Found:\n${offenders.join('\n')}`));
        }
        return ok(undefined);
    }
    /**
     ...
     */
    deriveRouteSlugs(fromRelativePath) {
