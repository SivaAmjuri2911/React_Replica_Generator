 ...
 */
export class TestFileParityRule {
    name = 'TestFileParityRule';
    async evaluate(context) {
        ...
        if (!solutionTestFile || !testcaseTestFile) {
            return err(new RuleViolationError(this.name, `Could not locate a *.test.jsx file in both solution_code (${generatedProject.solutionCodePath}) and testcase (${generatedProject.testcasePath})`));
        }
        ...
        if (solutionContent.value !== testcaseContent.value) {
            return err(new RuleViolationError(this.name, 'The test file in testcase must be byte-identical to the validated test file in solution_code'));
        }
        return ok(undefined);
    }
    async findTestFile(fileSystem, rootDir) {
