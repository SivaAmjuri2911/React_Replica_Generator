 ...
 */
export class ColorPaletteCompletenessRule {
    name = 'ColorPaletteCompletenessRule';
    async evaluate(context) {
        ...
        const relevantFiles = filesResult.value.filter((filePath) => /\.(jsx?|css|md|json)$/.test(filePath));
        const leftovers = [];
        ...
        if (leftovers.length > 0) {
            return err(new RuleViolationError(this.name, `Old-palette colors must not remain anywhere in solution_code. Found:\n${leftovers.join('\n')}`));
        }
        return ok(undefined);
    }
}
 ...
 */
function containsColorToken(lowerContent, lowerToken) {
