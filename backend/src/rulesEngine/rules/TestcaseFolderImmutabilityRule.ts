 ...
 */
export class TestcaseFolderImmutabilityRule implements GenerationRule {
  readonly name = 'TestcaseFolderImmutabilityRule';
  ...
  async evaluate(context: RuleEvaluationContext): Promise<Result<void, RuleViolationError>> {
    ...
    if (!baseFilesResult.ok) {
      return err(
        new RuleViolationError(this.name, `Could not list base testcase files: ${baseFilesResult.error.message}`)
      );
    }
    ...
    for (const baseFilePath of baseFilesResult.value) {
      ...
      const isExemptFromByteMatch =
        fileName === PACKAGE_JSON_FILENAME || TEST_FILE_EXTENSION_PATTERN.test(fileName);
    ...
    if (mismatches.length > 0) {
      return err(
        new RuleViolationError(
          this.name,
          `The testcase folder must be untouched except package.json's name field and the test file. Violations:\n${mismatches.join('\n')}`
        )
      );
    }
