 ...
 */
export async function findReadmeFile(fileSystem, projectDir) {
    const listResult = await fileSystem.listFilesRecursive(projectDir);
    if (!listResult.ok) {
        return undefined;
    }
    return listResult.value.find((filePath) => path.dirname(filePath) === projectDir && README_PATTERN.test(path.basename(filePath)));
}
