 ...
 */
export class ValidateImportResolutionStep {
    name = 'ValidateImportResolutionStep';
    fileSystem;
    ...
    async execute(context) {
        ...
        if (offenders.length > 0) {
            throw new RuleViolationError(this.name, `A generated file imports something that doesn't exist on disk — almost always a renamed/generated component whose name doesn't exactly match its actual file (a typo, a doubled letter, or a find/replace that fired twice). Every import must resolve to a real file. Found:\n${offenders.join('\n')}`);
        }
    }
    async resolveImport(fromDir, importPath) {
