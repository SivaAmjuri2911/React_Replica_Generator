 ...
 */
export class TransformSolutionCodeStep {
    name = 'TransformSolutionCodeStep';
    fileSystem;
    textTransformation;
    logger;
    ...
    async execute(context) {
        ...
        for (const rename of spec.fileRenames) {
            const moveResult = await this.fileSystem.moveFile(path.join(destination, rename.fromRelativePath), path.join(destination, rename.toRelativePath));
            if (!moveResult.ok) {
        ...
        if (!(await findReadmeFile(this.fileSystem, destination))) {
            throw new FileSystemError(`solution_code has no readme file (README.md / readme.md, any casing) at its root after ` +
                ...
                `its content as the platform's question_text.`, { destination });
        }
