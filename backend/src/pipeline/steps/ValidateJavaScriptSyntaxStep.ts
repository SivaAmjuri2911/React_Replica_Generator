...
const LOADER_BY_EXTENSION: Record<string, 'jsx' | 'tsx' | 'js' | 'ts'> = {
  '.jsx': 'jsx',
  '.tsx': 'tsx',
  '.js': 'js',
  '.ts': 'ts',
 ...
 */
export class ValidateJavaScriptSyntaxStep implements PipelineStep {
  readonly name = 'ValidateJavaScriptSyntaxStep';
  ...
  async execute(context: PipelineContext): Promise<void> {
    ...
    if (offenders.length > 0) {
      throw new RuleViolationError(
        this.name,
        `Generated code that isn't valid JavaScript/JSX — this always means a manually-authored file's content or a chained text replacement corrupted the code (e.g. a leftover "--- FILE: path ---" header, or a display-text replacement that also matched inside a variable/function name and broke it). Found:\n${offenders.join('\n')}`
      );
    }
  }

  private loaderFor(filePath: string): 'jsx' | 'tsx' | 'js' | 'ts' | undefined {
  ...
  private summarizeEsbuildError(cause: unknown): string {
    const errors = (cause as { errors?: Array<{ text?: string }> })?.errors;
