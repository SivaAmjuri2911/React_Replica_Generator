...
/** Phase 1 of phased drafting — structure only, no manually-authored file contents. */
export const scenarioSpecStructureDraftSchema = z.object({
    scenarioName: z.string(),
    outputFolderBaseName: z.string(),
    testPrefix: z.string(),
    textReplacements: z.array(textReplacementSchema),
    ...
    colorSwaps: z.array(colorSwapSchema),
    transformableRelativePaths: z.array(z.string()),
    manuallyAuthoredRelativePaths: z.array(z.string()),
});
/** Phase 2 of phased drafting — full content for a batch of manually-authored files. */
export const manuallyAuthoredFilesBatchSchema = z.object({
    manuallyAuthoredFiles: z.array(manuallyAuthoredFileSchema),
});
...
5. TEXT SPLIT ACROSS "<br />" — if a label is split by "<br />", write replacements/assertions per line, not as one contiguous phrase.

6. NEVER start file content with "--- FILE: path ---" headers — begin directly with real source (import, comment, etc.).

7. ROUTE AND COMPONENT NAMES — imports, JSX tags, and paths must match the exact basenames from fileRenames and textReplacements in the supplied structure.

Respond with ONLY the structured batch — no prose outside the schema fields.`;
...
export function buildScenarioSpecStructureUserPrompt(request) {
    return [
        buildScenarioSpecDraftUserPrompt(request),
        '',
 ...
 */
export function buildManuallyAuthoredFilesBatchUserPrompt(request, structure, batchPaths) {
    const structureSummary = {
        ...
        manuallyAuthoredRelativePaths: structure.manuallyAuthoredRelativePaths,
    };
    const baseFilesSection = batchPaths
        .map((targetPath) => {
            const baseFile = resolveBaseSourceFile(request.baseSolutionCodeFiles, structure, targetPath);
            if (!baseFile) {
                return `--- FILE: ${targetPath} ---\n(no matching base file — write from scratch using the structure)`;
            }
            return `--- FILE: ${baseFile.relativePath} ---\n${baseFile.contents}`;
        })
        .join('\n\n');
    return [
        'Write the complete new contents for ONLY the manually-authored files in this batch.',
        'Use the already-decided transformation structure below — do not change scenarioName, testPrefix, replacements, or renames.',
        '',
        'TRANSFORMATION STRUCTURE (already finalized):',
        JSON.stringify(structureSummary, null, 2),
        '',
        'Paths to write in THIS batch (each must appear exactly once in your output):',
        batchPaths.map((path) => `- ${path}`).join('\n'),
        '',
        'Original base file contents for reference (paths may differ when a file was renamed):',
        '',
        baseFilesSection,
    ].join('\n');
}
 ...
 */
export function resolveBaseSourceFile(baseSolutionCodeFiles, structure, targetPath) {
    const direct = baseSolutionCodeFiles.find((file) => file.relativePath === targetPath);
    if (direct) {
        return direct;
    }
    const rename = structure.fileRenames.find((entry) => entry.toRelativePath === targetPath);
    if (rename) {
...
export function buildScenarioSpecRevisionUserPrompt(request, previousDraft, failureSummary) {
    const filesSection = request.baseSolutionCodeFiles
        .map((file) => `--- FILE: ${file.relativePath} ---\n${file.contents}`)
        .join('\n\n');
    return [
