/**
 * @typedef {object} StreamedChatCompletionResult
 * @property {string} content
 * @property {string} refusal
 * @property {string|undefined} finishReason
 * @property {number|undefined} maxCompletionTokensUsed The max_completion_tokens value actually used for the request that produced this result — not necessarily what the caller originally asked for, see wasReducedForAffordability.
 * @property {boolean} wasReducedForAffordability True when the original request was rejected as unaffordable and this result comes from the automatic reduced-budget retry.
 */
 ...
 */
export async function streamChatCompletion(client, params, logger) {
    try {
        ...
        const reducedMaxTokens = requestedTokens !== undefined ? Math.min(affordableTokenLimit, requestedTokens) : affordableTokenLimit;
...
async function streamOnce(client, params, wasReducedForAffordability) {
...
function extractAffordableTokenLimit(cause) {
