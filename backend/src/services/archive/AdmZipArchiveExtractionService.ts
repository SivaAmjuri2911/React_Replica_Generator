...
import type { ArchiveExtractionService } from './ArchiveExtractionService.js';
import { err, ok, type Result } from '../../shared/Result.js';
import { FileSystemError } from '../../domain/errors/GenerationError.js';

export class AdmZipArchiveExtractionService implements ArchiveExtractionService {
  async extractZip(zipFilePath: string, destinationDir: string): Promise<Result<void, FileSystemError>> {
    try {
      ...
      return err(
        new FileSystemError(`Failed to extract zip "${zipFilePath}" to "${destinationDir}"`, {
          zipFilePath,
          destinationDir,
          cause,
        })
   ...
   */
  private async flattenSingleWrapperFolder(destinationDir: string): Promise<void> {
