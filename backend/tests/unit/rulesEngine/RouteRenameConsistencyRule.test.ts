...
import type { FileRename } from '../../../src/domain/models/TextReplacement.js';

function buildSpec(fileRenames: readonly FileRename[]): ScenarioSpec {
  return {
...
describe('RouteRenameConsistencyRule', () => {
  ...
  const fileRenames: readonly FileRename[] = [
  ...
  it('fails when a renamed page\'s route path still uses the base scenario\'s original literal', async () => {
    ...
    fileSystem.seed(
      `${generatedProject.solutionCodePath}/src/App.jsx`,
      '<Route path="/pod" element={<ItemsPage />} />\n<Route path="/providers" element={<ProvidersPage />} />'
    );
