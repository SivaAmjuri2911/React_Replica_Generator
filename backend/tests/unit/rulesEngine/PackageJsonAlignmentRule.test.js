...
describe('PackageJsonAlignmentRule', () => {
    ...
    it('fails when one package.json has a different name', async () => {
        const fileSystem = new InMemoryFileSystemService();
        const generatedProject = buildGeneratedProject();
        fileSystem.seed(`${generatedProject.prefilledCodePath}/package.json`, JSON.stringify({ name: 'ticket-management-project' }));
        fileSystem.seed(`${generatedProject.solutionCodePath}/package.json`, JSON.stringify({ name: 'WRONG-NAME' }));
        fileSystem.seed(`${generatedProject.testcasePath}/package.json`, JSON.stringify({ name: 'ticket-management-project' }));
        const result = await rule.evaluate({ spec: buildSpec(), generatedProject, fileSystem });
